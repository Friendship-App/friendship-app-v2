...
class EventForm extends Component {
  ...
  async submit() {
    ...
    let eventData = {
      ...
      maxParticipants,
      participantsMix,
      hostId: this.props.auth.data.decoded.id,
    ...
    if (this.state.eventImage) {
      ...
      await getPreSignedUrl('EVENT', imageData)
        .then(url => {
  ...
  render() {
    ...
    const cities = this.props.locations.locationsList.map(city => {
    ...
    return (
      <ScrollView>
        ...
        >
          ...
          <Footer onPress={() => this.submit()}>
            <Text>{this.props.edit ? 'Update' : 'Create'}</Text>
          </Footer>
        </BottomLabelWrapper>
      </ScrollView>
    );
